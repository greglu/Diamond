...
from yammer_metrics import MetricsCollector

################################################################################


class TestMetricsCollector(CollectorTestCase):
    def setUp(self):
        config = get_collector_config('MetricsCollector', {})

        self.collector = MetricsCollector(config, None)

    def test_import(self):
        self.assertTrue(MetricsCollector)
    ...
    def test_should_work_with_real_data(self, publish_mock):
        ...
        metrics = {
            ...
            'jvm.memory.memory_pool_usages.Code Cache': 0.038289388020833336,
            'jvm.memory.memory_pool_usages.PS Eden Space': 0.1918924383560846,
            'jvm.memory.memory_pool_usages.PS Old Gen': 0.022127459689416828,
            'jvm.memory.memory_pool_usages.PS Perm Gen': 0.32806533575057983,
            'jvm.memory.memory_pool_usages.PS Survivor Space': 1.0,
            'jvm.daemon_thread_count': 10,
            'jvm.thread_count': 27,
            'jvm.fd_usage': 0.014892578125,
            'jvm.thread-states.timed_waiting': 0.5185185185185185,
            'jvm.thread-states.runnable': 0.0,
